...
router = APIRouter(
    prefix="/comite/mensajes",
    tags=["Mensajes - Comité Nacional"]
)

# ------------------------------------------------------
# Validar que solo comité nacional acceda
# ------------------------------------------------------
def verificar_comite(user: Usuario):
    if user.rol_fk != 3:  # 3 = Comité Nacional
        raise HTTPException(status_code=403, detail="Acceso restringido a Comité Nacional")
...
def obtener_mensajes(
    ...
    verificar_comite(current_user)

    if bandeja == "recibidos":
        # Mensajes dirigidos al rol comité o directamente al usuario comité
        mensajes = db.query(models.Mensaje).filter(
            (models.Mensaje.destino_rol == 3) | 
...
def obtener_mensaje(
    ...
    # Permiso: comité como destinatario (rol o usuario) o remitente
    if mensaje.remitente_id != current_user.id_usuario and \
       mensaje.destino_rol != 3 and mensaje.destino_id != current_user.id_usuario:
        raise HTTPException(status_code=403, detail="No tiene permiso para ver este mensaje")

    # Marcar como leído si es recibido
    if (mensaje.destino_rol == 3 or mensaje.destino_id == current_user.id_usuario) and not mensaje.leido:
        mensaje.leido = True
        db.commit()
