(() => {
    ...
    function formatDate(dateStr) {
        ...
        return isNaN(d) ? String(dateStr) : d.toLocaleString("es-CO", { timeZone: "America/Bogota" });
    }
    ...
    function cambiarBandeja(bandeja) {
        ...
        document.querySelectorAll("#tabsMensajes .nav-link").forEach(tab => tab.classList.remove("active"));
        document.getElementById(bandeja === "recibidos" ? "tabRecibidos" : "tabEnviados")?.classList.add("active");
    ...
    async function verMensaje(id) {
        try {
            ...
            btnResp.style.display = m.remitente_id !== obtenerMiId() ? "inline-block" : "none";
    ...
    function abrirModalRedactar(respuestaId = null) {
        ...
        divDestino.style.display = respuestaAId ? "none" : "block";
